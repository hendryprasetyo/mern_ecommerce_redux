...
export const deliverOrder = (order) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(
      `/api/auth/order/${order._id}/deliver`,
      {},
      config
    )
...
export const successOrder = (order) => async (dispatch, getState) => {
  try {
    dispatch({
    ...
    const { data } = await axios.put(
      `/api/auth/order/${order._id}/success`,
      {},
      config
    )

    dispatch({
      type: ORDER_SUCCESS_SUCCESS,
      payload: data,
    })
  } catch (error) {
    const message =
